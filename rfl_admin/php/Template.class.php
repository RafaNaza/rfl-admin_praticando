...
class Template {
	 ...
	 */
	public function __construct($filename, $accurate = false, $txt=false){
		$this->accurate = $accurate;
		if($txt){
			$this->loadtxt('.', $filename);
		}
		else{
	...
	public function addFile($varname, $filename){
		if(!in_array($varname, $this->vars))
			throw new Exception("addFile: var $varname não existe");
	...
	/**
	 */
	private function loadtxt($varname, $str) {
		$file_array = explode("\n", $str);
		$blocks = $this->recognize($file_array, $varname);
		$str = implode("", $file_array);
		if (empty($str)) throw new Exception("arquivo $varname está vazio");
		$this->setValue($varname, $str);
		$this->createBlocks($blocks);
	}
	...
	private function subst($varname) {
		...
		foreach($this->instances as $varname => $inst) foreach($this->properties[$varname] as $p){
			...
			if(method_exists($inst, "get$method"))
				$s = str_replace("{".$varname.'->'.$p."}", $inst->{"get".$method}(), $s);
			// Boolean accessor
			elseif(method_exists($inst, "is$method"))
				$s = str_replace("{".$varname.'->'.$p."}", $inst->{"is".$method}(), $s);
			elseif(property_exists($inst, "$p"))
				$s = str_replace("{".$varname.'->'.$p."}", $inst->{"$p"}, $s);
			else
				$s = str_replace("{".$varname.'->'.$p."}", @$inst->{"$p"}, $s);
